...
But:
```
✅ Vercel (auto-scaling, deploys, CDN)
✅ Neon PostgreSQL (serverless, auto-backup)
✅ Stripe webhooks via Svix (hosted)
```

**Ops: Zero.** You deploy; Vercel runs it.
...
Background jobs: Bull Queue (free, runs on Vercel)
