...
# Think Before Code — Drift Edition

**Don't code blind.** This skill forces structured self-reflection BEFORE the first line — not a questionnaire for the user, but a short plan that captures behavior, UI states, and pitfalls specific to Drift's stack.
...
- **What's in Drift already I should reuse?**
...
### Step 3 — Drift-Specific Patterns
...
- **Permissions?** (check `@repo/auth` session, no hardcoded roles)
...
- "Use Drizzle or raw SQL?" (always Drizzle in Drift)
...
## Drift Stack Checklist (Embedded in Step 3)
...
- ✅ Permissions: auth via `@repo/auth`, role checks explicit
...
- With your knowledge of Drift's stack
...
3. **Tech stack** — which Drift tools/patterns
