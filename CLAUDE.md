...
### Infrastructure
- **Database:** Postgres — reviewers should flag unparameterized queries and migration safety.
- **Deployment:** Vercel
- **Secrets:** .env file (dotenv) — must not be committed — never commit secrets; never log them; never include them in error messages.
